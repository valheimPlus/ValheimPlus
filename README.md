...
# ValheimPlus
A HarmonyX Mod aimed at improving the gameplay quality of Valheim. The mod includes several different main features that allow users to modify the stats of players, buildings and entities, and build and place objects with very high precision through a sophisticated system, while also allowing to modify already placed objects with equal precision. The goal is to provide V+ as a base modification for Valheim to increase quality of life, tweak the game's difficulty and in general, improve the player's experience. The mod also comes with a version and configuration control system for servers and users, enabling server owners to ensure that only players with the same configuration are able to join the server.
...
* Display warning message when attempting to place different crops too close to each other.
* Remember mouse position when Valheim game window loses focus (Windows only feature).
* Added hotkey options for foward and backward roll.

**Map**
* Display player map position by default.
...
**Fire sources**
* Allow all fire sources to retain max fuel.
* Alternatively, allow only torches to retain max fuel, while other fire sources continue to consume fuel.

**Game Difficulty**
* Modify the difficulty multipliers applied to health and damage of enemies based on the amount of connected players.
* Modify the range at which the game considers other players to be nearby.
* Add an additional amount of players to the player count for the difficulty calculation.
...
* Reconfigure the amount of rows and columns in player inventory and containers.
...
## Deconstruct feature
* This feature allows dismantling items craftable in that station back into their original components. It is accessed via an additional tab in every different crafting station (except cauldron). There are some added functionality and restrictions:
It allows changing the percentage of resources returned, and the percentage is rounded down when calculated. Value can be set from 0 to 100, and default is 100 which will return all materials.
Items require the same workbench level to deconstruct than they would require to be crafted.
It does not work with ammo, consumables, materials, trophies and ooze bombs.
If an was crafted using a trophy or resin to be crafted, these will not be returned.
...
1. Players can rotate the object selected in any direction while in the usual building mode by pressing certain hotkeys. The location of the object can be manipulated with the mouse:
   ScrollWheel + LeftAlt to rotate by 1 degree in Y axis.
   ScrollWheel + C to rotate by 1 degree in X axis.
   ScrollWheel + Z to rotate by 1 degree in Z axis.
2. Use the copy rotation hotkeys to copy the current rotation or apply the same rotation to the next object that is being built.
3. Build the object by clicking.

### Advanced Building Mode | Video : https://i.imgur.com/ddQCzPy.mp4
*How it works. All mentioned hotkeys can be modified.*
1. Players can freeze the item by pressing the configured key (F1 by default).
2. Players can modify the item position and rotation with the following key combinations:
   ...
   Numpad plus/minus to either increase or decrease speed, holding SHIFT to raise/lower by 10 instead of 1 (Pressing Shift at any moment in time increases the distance/rotation angle by *3)
3. Build the object by clicking.
...
1. Players can select the item with the configured key (Numpad0 is default).
2. Players can modify the item position and rotation with the following key combinations:
   Arrow Up/Down/Left/Right = moves the building object in the respective direction.
   Arrow Up/Down + Control = moves the building object up and down
   ScrollWheel = rotates the building object on the Y axis.
   ...
   Numpad plus/minus to either increase or decrease speed, holding SHIFT to raise/lower by 10 instead of 1 (Pressing Shift at any moment in time increases the distance/rotation angle by *3)
3. Press the confirmPlacementOfAdvancedEditingMode Hotkey to confirm the changes. (press abortAndExitAdvancedEditingMode HotKey to abort editing mode and reset the object).
...
The Config files name is supposed to be "valheim_plus.cfg" and it needs to be placed in "BepInEx\config".
...
Only the server configuration file (located in the server files) needs to be setup when hosting a server with V+.
